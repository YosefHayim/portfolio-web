...
import type { ColoredTech } from "@/types/tech";

export type TechIconData = ColoredTech & {
 id: number;
 className: string;
};
...
const FloatingIcon = ({
 ...
 className="group flex h-14 w-14 items-center justify-center rounded-2xl border border-[var(--border-subtle)] bg-[var(--bg-card)]/80 p-2 shadow-xl backdrop-blur-md transition-all duration-300 hover:scale-110 hover:border-[var(--border-hover)] hover:shadow-2xl md:h-16 md:w-16 md:p-2"
 transition={{
 ...
 <motion.span
 className="absolute -bottom-6 left-1/2 -translate-x-1/2 rounded-md bg-[var(--bg-card)] p-2 p-2 text-[10px] font-medium whitespace-nowrap text-(--text-secondary) opacity-0 shadow-lg transition-opacity duration-200 group-hover:opacity-100"
...
>(({ className, title, subtitle, icons, ...props }, ref) => {
 ...
 <motion.p
 animate={{ opacity: 1, y: 0 }}
 className="mx-auto max-w-xl text-base text-[var(--text-muted)] md:text-lg"
 initial={{ opacity: 0, y: 20 }}
