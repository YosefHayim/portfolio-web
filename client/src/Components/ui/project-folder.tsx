...
const ProjectCard = forwardRef<HTMLDivElement, FolderPreviewCardProps>(
 (
 {
 image,
 title,
 ...
 >
 <div className="flex flex-wrap justify-center gap-2 rounded-lg border border-[var(--border-subtle)] bg-[var(--bg-card)]/95 p-2 shadow-xl backdrop-blur-sm">
 {techStack.slice(0, 6).map((tech) => (
 <span
 className="rounded-full bg-[var(--bg-elevated)] p-2 p-2 text-[8px] font-medium text-(--text-secondary)"
 key={tech}
 >
 {tech}
 </span>
 ...
 {techStack.length > 6 && (
 <span className="rounded-full bg-[#05df72]/20 p-2 p-2 text-[8px] font-medium text-[#05df72]">
 +{techStack.length - 6}
 </span>
 )}
 </div>
...
const ImageLightbox = ({
 ...
 <div
 className={cn(
 "fixed inset-0 z-50 flex items-center justify-center p-2 md:p-2",
 )}
 onClick={handleClose}
 style={{
 ...
 <div
 className="border-t border-[var(--border-subtle)] bg-[var(--bg-card)] p-2 p-2 sm:p-2 sm:p-2 md:p-2 md:p-2 lg:p-2 lg:p-2"
 style={{
 opacity: animationPhase === "complete" && !isClosing ? 1 : 0,
 transform:
 animationPhase === "complete" && !isClosing
 ...
 key={s}
 className={cn(
 "shrink-0 rounded-full p-2 p-2 text-[8px] font-semibold uppercase sm:p-2 sm:text-[10px]",
 config.className,
 )}
 >
 {config.label}
 ...
 <p className=" line-clamp-1 text-xs text-[var(--text-muted)] sm:line-clamp-2 sm:text-sm">
 {currentProject.description}
 </p>
 )}
 ...
 <button
 className="cursor-pointer rounded-full bg-[var(--bg-elevated)] p-2 p-2 text-[10px] text-[var(--text-muted)] transition-all hover:bg-[#05df72]/20 hover:text-[#05df72] sm:p-2 sm:text-xs"
 ...
 <div className="flex flex-wrap items-center gap-2 p-2 sm:gap-2 md:gap-2">
 <div className="flex flex-wrap items-center gap-2 rounded-2xl bg-[var(--bg-surface)] p-2 p-2 sm:gap-2 sm:p-2 sm:p-2 md:gap-2 md:p-2">
 {projects.map((_, idx) => (
 <button
 className={cn(
 ...
 <a
 className="flex items-center gap-2 text-xs text-[var(--text-muted)] transition-colors hover:text-[var(--text-primary)] sm:text-sm"
 ...
 <Link
 className="flex items-center gap-2 text-xs font-medium text-[#05df72] transition-colors hover:text-[#04c566] sm:text-sm"
...
export const AnimatedFolder = ({
 ...
 <div
 className={cn(
 "group relative flex min-h-[320px] min-w-[280px] cursor-pointer flex-col items-center justify-center rounded-2xl border border-[var(--border-subtle)] bg-[var(--bg-card)] p-2 transition-all duration-700 ease-[cubic-bezier(0.16,1,0.3,1)] hover:border-[#05df72]/40 hover:shadow-2xl hover:shadow-[#05df72]/10",
 className,
 )}
 onClick={(e) => {
 e.stopPropagation();
 ...
 <h3
 className=" text-lg font-bold text-[var(--text-primary)] transition-all duration-500"
 style={{
 ...
 <div
 className="absolute bottom-4 left-1/2 flex -translate-x-1/2 items-center gap-2 text-xs font-semibold tracking-widest text-[var(--text-muted)]/50 uppercase transition-all duration-500"
 style={{
